...
//IP="164.92.67.249";
//const rasa_action_endpoint_url = `http://${IP}:5055/webhook`;
//const rasa_server_url = `http://${IP}:5005/webhooks/rest/webhook`;
//const handoff_server_url = `http://${IP}/webhooks/rest/webhook`; // UNUSED Human handoff

const botPic  = "./assets/img/botAvatar.png";
const userPic = "./assets/img/avatar.png";
let Respuesta="";
const action_name = "action_saludo";
...
$(document).ready(() => { 
  ...
  var session = localStorage.getItem('RASA_SESSION_ID');
  //console.log(session);
  ...
  setTimeout(() => {
    ...
    for (let i = 0; i < suggestions.length; i += 1) {
      $(`<div class="menuChips" data-payload='${suggestions[i].payload}'>${suggestions[i].title}</div>`)
        ...
        Respuesta+=`<div class="menuChips" data-payload='${suggestions[i].payload}'>${suggestions[i].title}</div>`;
    }
...
function createButtonsCarousel(cardsData) {
  let cards = "";
  for (let i = 0; i < cardsData.length; i ++) {

    const item = `
      <div class="carousel_cards in-left">
        <img class="cardBackgroundImage" src="${cardsData[i].imagen}" width="50px">
        <div class="cardFooter">
          <div class="cardDescription">
            
              <div class="menuChips" data-payload="${cardsData[i].payload}">${cardsData[i].title}</div>
...
function showButtonsCarousel(cardsToAdd) {
  const cards = createButtonsCarousel(cardsToAdd);
...
function getUserID(){
  ...
  //const response = `/saludo{"UserID":"123"}`;
  Repuesta= localStorage.getItem("bot");
  //const savedId = storage.getItem(IdSession);
  

    //Repuesta=null;
    //console.log(Repuesta);
  if(Repuesta!=null){
    $(Repuesta).appendTo(".chats");
       
  }else{
  const response = "/saludo";
...
function setBotResponse(response) {
  ...
  setTimeout(() => {
    ...
    if (response.length < 1) { // There is no response from Rasa or human
      const fallbackMsg = "Estoy teniendo problemas para comunicarme, por favor intenta más tarde :(.";
      const BotResponse = `
      ...
      for (let i = 0; i < response.length; i += 1) {
        ...
        if (Object.hasOwnProperty.call(response[i], "text")) { // Response contains "text"
          if (response[i].text != null) {
            ...
            /*const BotResponse = `
              ...
              <img class="botAvatar" src="${botPic}"/>
              <p id="${result1}" class="botMsg">${formatted_text}</p>
              <div id="clasificacion_${result1}" class="calificacion" style="display:flex;margin-top:10px;opacity:0;"><button class="material-icons">sentiment_very_satisfied</button>
              <button class="material-icons">sentiment_dissatisfied</button></div>
              <div class="clearfix"></div>
             `; */  
             ...
             localStorage.setItem("bot", Respuesta);
             

            $(BotResponse).appendTo(".chats").hide().fadeIn(fadeTime);
          
            $(BotResponse).append(function(){
            $("#"+result1).mouseover(function (e) { 
              $("#clasificacion_"+result1).css("opacity","0.9");
              e.preventDefault();
            });
    
            $("#clasificacion_"+result1).mouseover(function (e) { 
              $("#clasificacion_"+result1).css("opacity","0.9");
              
              e.preventDefault();
            });
    
            $("#"+result1).mouseout(function (e) { 
              $("#clasificacion_"+result1).css("opacity","0");
              e.preventDefault();
            });
    
            $("#clasificacion_"+result1).mouseout(function (e) { 
              $("#clasificacion_"+result1).css("opacity","0");
              e.preventDefault();
            });
          });
        ...
        if (Object.hasOwnProperty.call(response[i], "attachment")) { // Response contains "attachment"
          if (response[i].attachment != null) {
            ...
            if (response[i].attachment.type === "template") { // Attachment type is "video"
              const BotResponse = `
                <div class="video-container">
                  <iframe src="${response[i].attachment.payload.src}" frameborder="0" allowfullscreen></iframe>
                </div>`;
        ...
        if (Object.hasOwnProperty.call(response[i], "custom")) { // Response contains "custom"
          ...
          if(payload=="ButtonsCarusel"){
            const restaurantsData = response[i].custom.data;
            showButtonsCarousel(restaurantsData);
            return;
          }
...
function send(message) {
  ...
  $.ajax({
    ...
    contentType: "application/json",
    //data: JSON.stringify({next_action: action_name, tracker: {sender_id}}),
    data: JSON.stringify({ message:message, sender: sender_id1}),
    success(botResponse, status) {
      sessionStorage.setItem("cache_session",botResponse)
      ...
      //bot_utter(botResponse);
      setBotResponse(botResponse);
      //response_bot+=botResponse;
      
      //console.log("mensaje bot"+response_bot);
 ...
 */
 function requestHuman() {
  setUserResponse("Yo requiero un asistente humano.")
  send("/request_human");
}
...
$(document).on("click", ".cardDescription .menuChips", function () {
  const text = this.innerText;
  const payload = this.getAttribute("data-payload");
  setUserResponse(text);
  send(payload);
